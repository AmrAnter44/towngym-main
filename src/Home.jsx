        ...
        <div className='pt-4 md:flex md:flex-wrap flex-row-reverse gap-4 justify-center'>
          {offers.length > 0 ? (
            offers.map((offer) => (
              ...
              <div key={offer.id} className= {offer.id==1 ? ' md:w-80 text-white  offer mt-7 p-2 rounded-lg flex flex-col bg-blue-700 shadow-md border-b-8 border-white opacity-90' : ' md:w-80 text-blue-700  offer mt-7 p-2 rounded-lg flex flex-col bg-white shadow-md border-b-8 border-blue-600 opacity-90'}        >
                <h3 className='p-2 font-bold  text-xl gymfont'><i className="fa-solid fa-dumbbell pr-2"></i>  {offer.duration}</h3>
                <div className='flex justify-between'>
                  <h3 className='p-2 font-bold  text-lg line-through'><i className="fa-solid fa-tag pr-1"></i>{offer.price.split('.00')} EGP</h3>
