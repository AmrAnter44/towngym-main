...
//         <h2 className='font-semibold text-pretty'> {coache.name} </h2>
//         <h3> {coaches.title}</h3>
//         {userToken !== sessionStorage.getItem('token') ? <button className='text-xl btn m-5 text-blue-700' onClick={()=>deleteCoaches(coache.id)}>{DeletingCoaches && DelIdCoaches==coache.id ? <span><i class="fa-solid fa-check text-blue-700"></i> <span className='text-blue-700'>Done</span> </span> : <span className='text-blue-600'>Delete</span> }</button>
// : null}
          ...
          <Slider className="pb-5 gap-2" {...settings}>
            {coaches?.length > 0 ? (
              coaches.map((coache) => (
                <div
                  key={coache.id}
                  className="w-full bg-blue-800 bg-opacity-70 text-white justify-between flex flex-row pt-2 gap-2 border"
                >
                  <div className="w-full flex flex-row">
                    <div className="w-full top-5 justify-center">
                    <img 
  className="w-40"
  src={coache.img.startsWith("data:image") ? coache.img : `data:image/png;base64,${coache.img}`}
  alt="Coach Img"
/>
                    </div>
                    <div className="lg:p-6 lg:m-6 font-bold text-2xl justify-center flex flex-col gap-4 text-center">
                      ...
                      {userToken === sessionStorage.getItem("token") ? (
                        <button
                          className="text-xl btn m-5 text-blue-700"
                          onClick={() => deleteCoaches(coache.id)}
                        >
                          {DeletingCoaches && DelIdCoaches === coache.id ? (
                            <span>
